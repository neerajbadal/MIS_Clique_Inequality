...
#     rc('font', family='serif')
    ...
    outDataFile = "D:/Mtech/FY/SEM2/Latex_Assignment/proj_out_greedy_approx_v4_1.dat"
...
#     print(da_pd.loc[idx_][0])
    
    new_pd = da_pd.loc[idx_]
...
#     plt.plot(da_pd[0],da_pd[2],marker='o',label=r'Brute Force $\gamma(\mathcal{R})$')
#     plt.plot(da_pd[0],da_pd[4],marker='+',label=r'Greedy $\gamma(\mathcal{R})$')
#     plt.plot(da_pd[0],da_pd[3],marker='*',label=r'DnC $\gamma(\mathcal{R})$')
#     plt.xlabel(r"$|\mathcal{R}|$",fontsize=18)
#     plt.ylabel(r"$\gamma(\mathcal{R})$",fontsize=20)
#     plt.title(r"$\gamma(\mathcal{R})$ for $|\mathcal{R}| \in [5,24]$ from random inputs",
#               fontsize=22,y=1.01)
#     ax = plt.gca()
#     ax.tick_params(axis = 'both', which = 'major', labelsize = 18)
#     ax.tick_params(axis = 'both', which = 'minor', labelsize = 18)
